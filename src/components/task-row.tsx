...
export function TaskRow({ task, tags = [] }: { task: Task; tags?: string[] }) {
  ...
  // Let the row finish animating before the server action pulls it out of the
  // list, otherwise it just blinks out of existence.
    ...
    >
      <div className="flex items-center gap-3.5 px-4 py-3">
        <button
          ...
          className={`mt-1.5 size-3 shrink-0 rounded-full transition-all hover:scale-125 focus:outline-none focus-ring priority-dot-${task.priority} ${task.status === "doing" ? "pulse-flame" : ""}`}
          style={{ background: PRIORITY_COLORS[task.priority] }}
        />

        <div className="min-w-0 flex-1 flex flex-col justify-center">
          <p className={`text-[14px] font-medium leading-snug ${closed ? "text-muted line-through opacity-70" : "text-ink"}`}>
            {task.title}
          </p>
          ...
          {noteOpen ? (
            ...
            >
              <input
                ...
                className="min-w-0 flex-1 rounded-xl border border-line bg-raised px-3 py-1.5 text-[13px] outline-none placeholder:text-faint focus-ring"
              />
              <button className="rounded-xl bg-accent px-3.5 text-[13px] font-semibold text-white transition-opacity hover:opacity-90">
                Log
              </button>
            </form>
          ) : null}
        ...
        <div className="flex shrink-0 items-center gap-1 opacity-0 transition-all duration-300 translate-x-2 focus-within:translate-x-0 focus-within:opacity-100 group-hover:translate-x-0 group-hover:opacity-100">
          {closed ? (
...
function RowButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className={`grid size-7 place-items-center rounded-lg border transition-all duration-200 active:scale-90 hover:scale-105 ${
        accent
          ...
          : "border-transparent bg-transparent text-muted hover:border-line hover:text-ink hover:bg-surface hover:shadow-xs"
      }`}
    >
      {children}
    </button>
